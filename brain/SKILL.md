...
## name: brain-structure description: >- Manages brain-style repo structure (areas, projects, resources, zettelkasten). Respects README for humans and AGENTS.md for agents; applies all AGENTS.md from root down to current path as an overlay. Use when working in areas, projects, resources, zettelkasten, when creating/updating structure or agent guidance, when initializing an uninitialized directory from the template, or when the user asks for a review or audit of repo conformance to this contract.

# Brain structure

Guidance for agents operating in a brain-style repository. Procedures live in the repo’s own files; this skill defines the contract and how to apply it.
...
| **AGENTS.md** | Agents | What agents need to know: scope, conventions, and how to operate within this path. |
...
- **Root** `AGENTS.md` — repo-wide layout, conventions, tooling. Always applies.
- **Intermediate** `AGENTS.md` (e.g. `areas/AGENTS.md`, `projects/AGENTS.md`) — applies when present, for everything under that directory.
- **Leaf** `AGENTS.md` (e.g. `areas/my-area/AGENTS.md`, `zettelkasten/AGENTS.md`) — scope-specific rules for that folder.
...
1. **When adding or changing structure** — Create or update README.md for humans and AGENTS.md for agents at the right scope. Keep procedures at the appropriate level (repo-wide in root, scope-specific in that scope’s AGENTS.md).
...
1. **Audit against the contract:**
   ...
   - **Split:** In each pair, does README hold human-facing content (what this is, how to use it, how it connects) and AGENTS.md hold agent-facing content (scope, conventions, how to operate)? Flag content that belongs in the other file.
...
1. **Copy template contents** — Copy all files and directories from `assets/brain-template/` into the target, preserving structure. Do not copy the `brain-template` folder itself; copy its contents so that AGENTS.md, README.md, areas/, projects/, resources/, scripts/, zettelkasten/, Makefile, pyproject.toml, .pre-commit-config.yaml, and .gitignore end up at the target root.
1. **Follow-up** — After copying, point the user to the new root README.md (quick start: rename project in pyproject.toml if desired, run `make install`, then customize areas/projects/zettelkasten).
